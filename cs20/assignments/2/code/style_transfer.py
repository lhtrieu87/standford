...
class StyleTransfer(object):
    ...
    def _style_loss(self, A):
        ...
        ###############################
        nlayers = len(A)
        E = [self._single_style_loss(A[i], getattr(self.vgg, self.style_layers[i])) for i in range(nlayers)]

        self.style_loss = sum([self.style_layer_w[i] * E[i] for i in range(nlayers)])
        ###############################
